...
export default function Patients() {
  ...
  const [form, setForm] = useState({ name: '', age: '', type: 'Ferida' })
  ...
  const addPatient = async (e) => {
    ...
    const created = await createPatient({ name: form.name, age: Number(form.age), type: form.type })
    setPatients((list) => [created, ...list])
    showToast('Paciente cadastrado com sucesso.')
    setForm({ name: '', age: '', type: 'Ferida' })
    setShowForm(false)
    setPage(1)
  }
    ...
    <div className="page">
      ...
      {showForm && (
        <form className="panel patients-form" onSubmit={addPatient}>
          ...
          <div className="form-field">
            <label>Idade</label>
            <input type="number" min="0" value={form.age} onChange={(e) => setForm((f) => ({ ...f, age: e.target.value }))} required />
          </div>
          <div className="form-field">
